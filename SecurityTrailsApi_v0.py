...
def functionCallMethod():
    ...
    parser.add_argument("-dsl", "--dsl",type = str, help="Request to details dsl domains method of securitytrails api")
    ...
    if args.ping:
        ...
        print("usage: SecurityTrailsApi.py [-h] for to select correct method")
...
def secTrailsApiIpDSLMethod(domain):
    ...
    try:
        ...
        ipNeighbors = "ips/list" 
        
        payload = {"query": "ptr_part = '%s'" % domain}
        
        response = requests.request("GET", URL_API_SECURITY_TRAILS + "%s" % ipNeighbors, json = payload, headers=HEADERS_API_SECURITY_TRAILS)
